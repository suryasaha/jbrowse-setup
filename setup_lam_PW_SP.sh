...
bin/flatfile-to-json.pl --gff ~/work/jbrowse_setup/las_PW_SP/LAM_PW_SP.gbrowse.gff3 --type contig  --trackLabel contig --key "Contigs" --trackType CanvasFeatures --out data/json/las_PW_SP/ --clientConfig '{"description" : "note,description", "label" : "name,id", "color" : "green" }' --metadata '{"description": "Contigs from Genbank in the pseudomolecule", "category": "General" }' --config '{ "glyph": "JBrowse/View/FeatureGlyph/ProcessedTranscript" }' 
bin/flatfile-to-json.pl --gff ~/work/jbrowse_setup/las_PW_SP/LAM_PW_SP.gbrowse.gff3 --type gap  --trackLabel gap --key "TIGR linker" --trackType CanvasFeatures --out data/json/las_PW_SP/ --clientConfig '{"description" : "note,description", "label" : "name,id", "color" : "green" }' --metadata '{"description": "TIGR linker joining contigs from Genbank in the pseudomolecule", "category": "General" }' --config '{ "glyph": "JBrowse/View/FeatureGlyph/ProcessedTranscript" }'
