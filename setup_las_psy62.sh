...
# Pathways
bin/flatfile-to-json.pl --gff ~/work/jbrowse_setup/las_psy62/NC_012985.gbrowse.gff3 --type MetPathway  --trackLabel MetPathway --urltemplate "http://www.genome.jp/dbget-bin/www_bget?las:{name}" --key "Metabolic Pathways" --trackType CanvasFeatures --out data/json/las_psy62/ --clientConfig '{ "label" : "name,id", "color" : "orange" }' --metadata '{"description": "Metabolic Pathway predictions with links to KEGG", "category": "Predictions" }' --config '{"menuTemplate" : [{"label" : "View details",}, {"label" : "Highlight this gene"}, {"label" : "Ask KEGG about {name}", "iconClass" : "dijitIconDatabase", "action" : "newWindow", "url" : "http://www.genome.jp/dbget-bin/www_bget?las:{name}"}, {"label" : "Ask BioCyc about {name}", "iconClass" : "dijitIconDatabase", "action" : "newWindow", "url" : "http://biocyc.org/LASI537021/NEW-IMAGE?type=GENE-IN-PWY&object={name}"}] }'
...
# repeats
bin/flatfile-to-json.pl --gff ~/work/jbrowse_setup/las_psy62/NC_012985.gbrowse.gff3 --type repeat:RepeatScout  --trackLabel repeat:RepeatScout --key "RepeatScout repeats" --trackType CanvasFeatures --out data/json/las_psy62/ --clientConfig '{"description" : "description", "label" : "name,id", "color" : "green" }' --metadata '{"description": "RepeatScout predictions", "category": "Predictions" }'

bin/flatfile-to-json.pl --gff ~/work/jbrowse_setup/las_psy62/NC_012985.gbrowse.gff3 --type repeat:RepeatMasker  --trackLabel repeat:RepeatMasker --key "RepeatMasker repeats" --trackType CanvasFeatures --out data/json/las_psy62/ --clientConfig '{"description" : "description", "label" : "name,id", "color" : "green" }' --metadata '{"description": "RepeatMasker matches to known repeats in RepBase", "category": "Predictions" }'
